...
async function refresh(refreshToken, onRotate) {
  const r = await formPost(`${MSA}/token`, {
  ...
  // Save a rotated Microsoft token even if a later Xbox/Minecraft request fails.
  if(r.data.refresh_token && onRotate)await onRotate(r.data.refresh_token);
