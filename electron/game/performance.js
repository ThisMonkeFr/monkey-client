...
async function sync(profile,onProgress=()=>{},io=require('./io')){
 ...
 for(const file of await fs.readdir(mods))if(file.endsWith('.jar')&&!file.startsWith('monkey-perf-')){
  for(const id of await require('./mod-ids').readIds(path.join(mods,file)))userIds.add(id);
 }
 const selected=selection(profile).filter(m=>!userIds.has(m.modId)&&!(profile.mods||[]).some(u=>u.projectId===m.projectId));
 ...
 for(const row of selected){
  ...
  if(stat){const hash=await digest(target);if(hash===row.sha512)continue;if(hash!==owned.get(row.fileName)?.sha512)throw Error('A custom optimization file occupies '+row.fileName+'. Move it before changing the preset.');}
