...
(async()=>{
 ...
 await page.goto(pathToFileURL(path.resolve('renderer/index.html')).href);await page.waitForSelector('body.boot-ready');
 ...
 const result=await page.evaluate(()=>({style:document.documentElement.dataset.menuStyle,overflow:document.documentElement.scrollWidth>innerWidth,booting:document.body.classList.contains('booting')}));
 if(result.style!=='monkey'||result.overflow||result.booting||errors.length)throw Error(JSON.stringify({result,errors}));
