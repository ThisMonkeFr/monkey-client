...
(async()=>{
 ...
 await fs.mkdir('build/ui-check',{recursive:true});await page.screenshot({animations:'disabled',path:'build/ui-check/play.png'});
 await page.getByRole('button',{name:'Screenshots',exact:true}).click();await page.getByRole('heading',{name:'Screenshots',exact:true}).waitFor();await page.screenshot({animations:'disabled',path:'build/ui-check/screenshots.png'});
 await page.getByRole('button',{name:'Launcher settings',exact:true}).click();await page.screenshot({animations:'disabled',path:'build/ui-check/settings.png'});
 await page.evaluate(()=>{closeModal();Object.assign(S.theme,{accent:'#ff7900',bg:'#311409',panel:'#662814'});applyTheme();go('play');});await page.screenshot({animations:'disabled',path:'build/ui-check/orange.png'});
 await page.setViewportSize({width:1000,height:680});await page.screenshot({animations:'disabled',path:'build/ui-check/minimum.png'});
