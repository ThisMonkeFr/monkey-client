...
async function main(){
 ...
 try{
  ...
  for(const marker of ['RESOURCES_OK','WORLD_JOINED','PAUSE_MENU','MENU_OPEN','MODULE_GRID','INPUT_PASS','VANILLA_MOVEMENT_PASS','NATIVE_FRIENDS','NATIVE_GALLERY','NATIVE_SKINS','SCREENSHOT_CHAT_PASS','WORLD_PASS'])if(!text.includes('MONKEY_QA_'+marker))throw Error('Missing world check: '+marker);
